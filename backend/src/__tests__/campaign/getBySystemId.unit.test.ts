...
describe("campaignController.getBySystemId", () => {
  ...
  it("should return all campaigns by systemId", async () => {
    const mockCampaigns = [
      {
        id: 1,
        title: "New Campaign",
    ...
    (CampaignServices.getBySystemId as jest.Mock).mockResolvedValue(
      mockCampaigns
    );

    await campaignController.getBySystemId(req as Request, res as Response);

    expect(CampaignServices.getBySystemId).toHaveBeenCalledWith(
      (req.params as { systemId: string }).systemId
    );
  ...
  it("should return 404 if campaign not found", async () => {
    (CampaignServices.getBySystemId as jest.Mock).mockResolvedValue(null);
  ...
  it("should return 500 if there is a server error", async () => {
    (CampaignServices.getBySystemId as jest.Mock).mockRejectedValue(
