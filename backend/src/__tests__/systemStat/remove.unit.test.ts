...
describe("systemStatController.remove", () => {
  ...
  it("should return 200 and the removed systemStat if found", async () => {
    const mockSystemStat = {
      id: 1,
      systemId: 1,
      statId: 1,
    };
    (systemStatServices.remove as jest.Mock).mockResolvedValue(mockSystemStat);

    await systemStatController.remove(req as Request, res as Response);

    expect(systemStatServices.remove).toHaveBeenCalledWith("1");
  ...
  it("should return 404 if the systemStat is not found", async () => {
    (systemStatServices.remove as jest.Mock).mockResolvedValue(null);
  ...
  it("should return 500 if an error occurs", async () => {
    (systemStatServices.remove as jest.Mock).mockRejectedValue(
